...
const SuggestedPosts = ({className , header, lnews =[], tags}) => {
  return (
    <div className={`w-full shadow-[rgba(7,_65,_210,_0.1)_0px_9px_30px] rounded-lg p-4 ${className}`}>
      ...
      {lnews.map((item)=>(
        <div key={item._id} className='flex space-x-3 flex-nowrap items-center'>
            <img className='aspect-square object-cover rounded-lg w-[100px] h-[70px]' src={item?.photo ? stables.UPLOAD_FOLDER_BASE_URL + item.photo : images.samplePostImage} alt={item.title} />
            ...
            <div className='text-sm font-roboto text-dark-hard font-medium'>
                ...
                <span className='text-xs opacity-60'>
                    {new Date(item.createdAt).toLocaleDateString("en-US", {day:"numeric", month: "short", year: "numeric" })} {/* for string to data convertion*/}
                </span>
            </div>
        </div>
      ))}
      ...
      {tags.length === 0 ? (<p className="text-slate-500 text-xs mt-2">There is no tags for this news</p>) : 
