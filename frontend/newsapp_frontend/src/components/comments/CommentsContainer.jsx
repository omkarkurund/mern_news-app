...
const CommentsContainer = ({className, logginedUserId , comments , postSlug}) => {
    ...
    const {mutate:mutateNewComment , isLoading: isLoadingNewComment}= useMutation(
      {
        ...
        onSuccess:()=>{
          ...
          queryClient.invalidateQueries(["news",postSlug])
        },
        onError:(error)=>{
    ...
    const updateCommentHandler =(value,commentId)=>{
      mutateUpdateComment({desc: value, commentId, token :userState.userInfo.token})
      setAffectedComment(null);
    };
  ...
  return (
    <div className={`${className}`}>
      <CommentForm btnLabel="Send" formSubmitHandler={(value)=> addCommentHandler(value)} loading={isLoadingNewComment} />
