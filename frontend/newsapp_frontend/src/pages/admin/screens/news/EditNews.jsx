...
import ArticleDetailSkeleton from '../../../articleDetails/components/ArticalDetailSkeleton';
...
const EditNews = () => {
    ...
    const { data, isSuccess, isLoading, isError } = useQuery({
      ...
      const {mutate: mutateUpdateNewsDetail,isLoading: isLoadingUpdateNewsDetail} = useMutation({
        mutationFn: ({updatedData, slug, token} )=>{
          return updateNews({ updatedData,slug, token });
        },
        onSuccess:(data)=>{
    ...
    setPhoto(file);
      ...
      //   mutateUpdateNewsDetail({updatedata, slug, token: userState.userInfo.token})
      // }
      ...
      const handleUpdateNews = async () => {
        ...
        if (!initialPhoto && photo) {
            console.log("Adding new photo to FormData");
            updatedata.append("postPicture", photo);
        } else if (initialPhoto && !photo) {
            const UrlToObject = async (url) => {
                let response = await fetch(url);
            ...
            const picture = await UrlToObject(stables.UPLOAD_FOLDER_BASE_URL + data?.photo);
            console.log("Adding existing photo to FormData");
            updatedata.append("postPicture", picture);
        }
    
        const documentData = JSON.stringify({ body , categories});
        console.log("Adding document to FormData:", documentData);
        updatedata.append("document", documentData);
    
        console.log("FormData prepared:", updatedata);
    
        mutateUpdateNewsDetail({ updatedData: updatedata, slug, token: userState.userInfo.token });
    };

    const handleDeleteImage= ()=>{
      if(window.confirm("Do you wnat to delete your news picture")){
        setInitialPhoto(null);
        setPhoto(null)
      }
    }
    
    let NewsDataLoaded = !isLoading && !isError;

  return (
    <div>
         {isLoading ? (<ArticleDetailSkeleton />) : isError ? <ErrorMessage message="Couldn't fetch the news detail" /> : (
        <section className='container mx-auto max-w-5xl flex flex-col px-5 py-5 lg:flex-row lg:gap-x-5 lg:items-start'>
          <article className='flex-1'>
            ...
            <button type='button' onClick={handleDeleteImage} className='w-fit bg-red-500 text-sm text-white font-semibold rounded-lg px-2 py-1 mt-5'>
                  Delete Image
            </button>
            ...
            <div className='my-5'>
              {NewsDataLoaded && <MultiSelectTagDropdown loadOptions={promiseOptions} defaultValue={data.categories.map(categoryToOption)} onChange={(newValue)=> setCategories(newValue.map((item)=> item.value))}/>}
            ...
            <div className='w-full'>
                 {NewsDataLoaded &&  <Editor content={data?.body} editable={true}  onDataChange={(data) => {
                    setBody(data);
                  }}/> }
               
            </div>

            <button disabled={isLoadingUpdateNewsDetail} type='button' onClick={handleUpdateNews} className='w-full bg-green-500 text-white font-semibold rounded-lg px-4 py-2 disabled:cursor-not-allowed disabled:opacity-70'>
              Update News
            </button>
         </article>
